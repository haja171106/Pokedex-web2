...
function PokemonCard({ pokemon }) {
  const normalGradient = color[pokemon.type] || "#ddd";

  return (
    <div
      className="rounded-xl p-4 text-neutral-950 w-36 h-52 shadow-md flex flex-col justify-around items-center cursor-pointer transition-all duration-300"
      style={{ background: normalGradient }}
      onMouseEnter={(e) => {
        e.currentTarget.style.background = normalGradient
          .split(", ")
          .reverse()
          .join(", ");
      }}
